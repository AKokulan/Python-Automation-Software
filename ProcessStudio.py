...
class ProcessStudioOutputTab:
    ...
    def add_row_button_call1(self,ps_process_tab):
        ...
        #calculate the number of rows in current table
        rows_tableframe=(len(children_windows))/4
        len_child_win=len(children_windows)
        print('number of widgets: ',len_child_win)
        ...
        # Find any checkbutton is checked
        checked=False
        for each in self.check_button_dict:
            if self.check_button_dict[each]==1:checked=True

        #if any checkbutton is checked, get the value from existing entry widgets in table frame and destory all the widegets
        widgets_values = []
        if checked==True:
            loop_count = 0
            for each in children_windows:
                print(each.winfo_class())
                if each.winfo_class()=='Entry':
                    widgets_values.append(each.get())
                each.destroy()

        print('list of values retrived for add\n', widgets_values)
        #create the row in after last row in the below scanarios
        # 1 . There is no rows available in the table
        # 2 . There is no checkbutton checked
        next_row=1
        if rows_tableframe>=1:next_row=int(rows_tableframe)+1
        print('row in table frame for add\n',rows_tableframe)
        print('next row in table frma eofr add \n',next_row)
        if (len_child_win)==0 or len(self.check_button_dict)==0 or checked!=True:
            print('add the row next to last row')

            Label(fr_table, text=next_row, relief=FLAT, bg='gray87').grid(row=next_row, column=1)
            #create Entrybox for variablename
            ent_variable = Entry(fr_table, bg='snow',width=58)
            ...
            ent_variable = Entry(fr_table, bg='snow', width=118)
        ...
        #Add the row when the checkbutton is checked
        rows_count=len(widgets_values)/2
        if rows_count>0:
            row=1
            column=1
            loop_count=0
            for each in widgets_values:
                loop_count+=1
                #if loop_count%2>0:row+=1

                #add the Entrybox for variable name with it's value
                if loop_count % 2 > 0:
                    column = 2
                    Label(fr_table, text=row, relief=FLAT, bg='gray87').grid(row=row, column=1)

                    var_variable_name = StringVar()
                    var_variable_name.set(each)
                    ent_variable = Entry(fr_table,textvariable=var_variable_name, bg='gray', width=58)
                    ent_variable.grid(row=row, column=column)

                # add the Entrybox for variable value  with it's value and the checkbutton
                if loop_count % 2 == 0:
                    column = 3
                    var_variable_value = StringVar()
                    var_variable_value.set(each)
                    ent_variable = Entry(fr_table,textvariable=var_variable_value, bg='gray', width=118)
                    ent_variable.grid(row=row, column=column)

                    var_row_select=IntVar()
                    cb_row_select = Checkbutton(fr_table,variable=var_row_select)
                    cb_row_select.configure(command=lambda cb=cb_row_select, var=var_row_select: get_checkbutton_value(cb, var))
                    cb_row_select.grid(row=row, column=4)
                    print('row added: ',row)
                    row += 1

                #if the checkbutton is checked, add the row
                if int(row) in self.check_button_dict and self.check_button_dict[row]==1 and loop_count % 2 == 0:
                    print(self.check_button_dict)
                    print('row found in check button dict: ', row)

                    Label(fr_table, text=row, relief=FLAT, bg='gray87').grid(row=row, column=1)

                    ent_variable = Entry(fr_table, bg='gray', width=58)
                    ent_variable.grid(row=row, column=2)

                    ent_variable = Entry(fr_table, bg='gray', width=118)
                    ent_variable.grid(row=row, column=3)
        ...
        print(fr_table.winfo_screenwidth())


    def del_row_button_call1(self,ps_process_tab):
        ...
        #if any checkbutton is checked, get all the value from existing entry widgets in table frame and destory all the widegets
        widgets_values = []
        if checked==True:
            loop_count = 0
            for each in children_windows:
                print(each.winfo_class())
                if each.winfo_class()=='Entry' and (each.grid_info()['row'])+1!=row_checked:
                    widgets_values.append(each.get())
                each.destroy()

        print('list of values retrived for del\n',widgets_values)
    ...
    def del_row_button_call2(self,ps_process_tab):
        ...
        #calculate the number of rows in current table
        rows_tableframe=(len(children_windows))/3
        len_child_win=len(children_windows)
        ...
        if checked==True:
            loop_count = 0
            for each in children_windows:
                if each.grid_info()['row']==row_checked:
                    each.destroy()
